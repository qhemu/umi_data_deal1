"""
This script generates a dataset plan for training models from video recordings of robotic manipulations.
...
def main(input, output, tcp_offset, tx_slam_tag,
    ...
    if tx_slam_tag is None:
        ...
        assert path.is_file(), f"[red]tx_slam_tag.json file does not exist.[/red]"
    ...
    with ExifToolHelper() as et:
        for video_dir in video_dirs:
            ...
            if cam_serial in ignore_cam_serials:
                print(f"[yellow]Ignored {video_dir.name}[/yellow]")
                continue
            
            csv_path = video_dir.joinpath('camera_trajectory.csv')
            if not csv_path.is_file():
                print(f"[yellow]Ignored {video_dir.name}, no camera_trajectory.csv[/yellow]")
                continue
            
            pkl_path = video_dir.joinpath('tag_detection.pkl')
            if not pkl_path.is_file():
                print(f"[yellow]Ignored {video_dir.name}, no tag_detection.pkl[/yellow]")
                continue
            ...
            with av.open(str(mp4_path), 'r') as container:
                ...
                if fps is None:
                    ...
                    if fps != stream.average_rate:
                        print(f"[red]Inconsistent fps: {float(fps)} vs {float(stream.average_rate)} in {video_dir.name}[/red]")
                        exit(1)
    ...
    for vid_idx in unused_videos:
        print(f"[yellow]Warning: video {video_meta_df.loc[vid_idx]['video_dir'].name} unused in any demo[/yellow]")
    ...
    for cam_serial, gripper_ids in cam_serial_gripper_ids_map.items():
        ...
        if len(counter) != 1:
            print(f"[yellow]Warning: multiple gripper ids {counter} detected for camera serial {cam_serial}[/yellow]")
    ...
    for demo_idx, demo_data in enumerate(demo_data_list):
        ...
        if len(pose_interps) != n_gripper_cams:
            print(f"[yellow]Excluded demo {demo_idx} from left/right disambiguation.[/yellow]")
            continue
    ...
    print("[blue]Assigned camera_idx: right=0; left=1; non_gripper=2,3...[/blue]")
    ...
    for demo_idx, demo_data in enumerate(demo_data_list):
        ...
        for cam_idx, row in demo_video_meta_df.iterrows():
            ...
            if check_path.is_file():
                if not check_path.open('r').read().startswith('true'):
                    print(f"[yellow]Skipping {video_dir.name}, manually filtered with check_result.txt!=true[/yellow]")
                    continue

            csv_path = video_dir.joinpath('camera_trajectory.csv')
            if not csv_path.is_file():
                print(f"[yellow]Skipping {video_dir.name}, no camera_trajectory.csv.[/yellow]")
            ...
            if n_frames_lost > 10:
                print(f"[yellow]Skipping {video_dir.name}, {n_frames_lost} frames are lost.[/yellow]")
                dropped_camera_count[row['camera_serial']] += 1
                continue

            n_frames_valid = is_tracked.sum()
            if n_frames_valid < 60:
                print(f"[yellow]Skipping {video_dir.name}, only {n_frames_valid} frames are valid.[/yellow]")
                dropped_camera_count[row['camera_serial']] += 1
                continue
            ...
            pkl_path = video_dir.joinpath('tag_detection.pkl')
            if not pkl_path.is_file():
                print(f"[yellow]Skipping {video_dir.name}, no tag_detection.pkl.[/yellow]")
                dropped_camera_count[row['camera_serial']] += 1
                continue
            ...
            if len(df) != len(video_timestamps):
                print(f"[yellow]Skipping {video_dir.name}, video csv length mismatch.[/yellow]")
                continue

            ghi = row['gripper_hardware_id']
            if ghi < 0:
                print(f"[yellow]Skipping {video_dir.name}, invalid gripper hardware id {ghi}[/yellow]")
                dropped_camera_count[row['camera_serial']] += 1
                continue
            ...
            if ghi in gripper_id_gripper_cal_map:
                ...
                print(f"[yellow]Gripper id {ghi} not found in gripper calibrations {list(gripper_id_gripper_cal_map.keys())}. Falling back to camera serial map.[/yellow]")
            else:
            ...
            if gripper_det_ratio < 0.9:
                print(f"[yellow]Warning: {video_dir.name} only {gripper_det_ratio} of gripper tags detected.[/yellow]")
        ...
        if len(all_cam_poses) != n_gripper_cams:
            print(f"[yellow]Skipped demo {demo_idx}.[/yellow]")
            n_dropped_demos += 1
            continue
    ...
    print(f"[blue]{int(used_ratio*100)}% of raw data are used.[/blue]")
