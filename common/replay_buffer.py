...
class ReplayBuffer:
    ...
    def copy_from_store(cls, src_store, store=None, keys=None, 
        ...
        if store is None:
            ...
            if keys is None:
                keys = src_root['data'].keys()  # 'camera0_rgb'
            for key in keys:
                value = src_root['data'][key]  # (2315,224,224,3)
                cks = cls._resolve_array_chunks(  # (1, 224, 224, 3)
                    chunks=chunks, key=key, array=value)
                cpr = cls._resolve_array_compressor(
                    compressors=compressors, key=key, array=value)
                if cks == value.chunks and cpr == value.compressor:
                    # copy without recompression
                    this_path = '/data/' + key  # '/data/camera0_rgb'
    ...
    def _resolve_array_chunks(cls,
            chunks: Union[dict, tuple], key, array):  # key:'camera0_rgb', array:<zarr.core.Array '/data/camera0_rgb' (2315, 224, 224, 3) uint8>
        cks = None  # (1, 224, 224, 3)
